import React, { useState } from 'react'

const App = () => {

  const [title, setTitle] = useState("")
  const [details, setDetails] = useState("")
  const [task, setTask] = useState([])

  const submitHandler = (e) => {
    e.preventDefault()

    const copyTask = [...task]
    copyTask.push({ title, details })

    setTask(copyTask)
    setTitle("")
    setDetails("")
  }

  const deleteNote = (index)=>{
    const copyTask = [...task]
    
    copyTask.splice(index,1)
    
    setTask(copyTask)
  }

  return (
    <div className="h-screen lg:flex bg-black text-white">

      {/* FORM */}
      <form
        onSubmit={submitHandler}
        className="flex items-start flex-col gap-4 p-10 lg:w-1/2"
      >
        <h1 className="text-4xl font-bold">Add Notes</h1>

        <input
          type="text"
          placeholder="Enter Notes Heading"
          className="px-5 w-full h-20 py-2 border-2 outline-none rounded font-medium text-2xl"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
        />

        <textarea
          className="px-5 w-full h-32 py-2 border-2 outline-none rounded font-medium text-xl"
          placeholder="Write Details"
          value={details}
          onChange={(e) => setDetails(e.target.value)}
        />

        <button className="bg-white text-black w-full px-5 py-2 font-bold text-2xl border-2 rounded">
          Add notes
        </button>
      </form>

      {/* NOTES */}
      <div className="p-10 lg:w-1/2 lg:border-l-2 border-gray-700 h-[90%] lg:overflow-y-auto no-scrollbar">
        <h1 className="text-4xl font-bold">Recent notes</h1>

        <div className="flex gap-5 flex-wrap mt-5 h-full items-start overflow-y-auto overflow-x-hidden no-scrollbar">
          {task.map((element, index) => (
            <div
              key={index}
              className="relative h-52 w-40 rounded-xl text-black
                         bg-white px-7 py-7 shadow-md bg-cover bg-center cursor-pointer bg-[url('data:image/jpeg;base64,')] "
            >

              {/* ✅ ONLY TEXT SCROLLS */}
              <div className="overflow-y-auto no-scrollbar h-[28.75] pr-1">
                <h3 className="leading-tight text-xl font-bold">
                  {element.title}
                </h3>

                <p className="mt-2 leading-tight font-medium text-gray-700">
                  {element.details}
                </p>
              </div>

              {/* ✅ DELETE BUTTON FIXED */}
              <button onClick={
                ()=>{ deleteNote(index)
              }}
                className="absolute bottom-3 left-1/2 -translate-x-1/2
                           bg-red-500 hover:bg-red-600 text-white
                           px-3 py-1 text-sm rounded-md cursor-pointer active:scale-95 transition-all"
              >
                Delete
              </button>

            </div>
          ))}
        </div>
      </div>
    </div>
  )
}

export default App
